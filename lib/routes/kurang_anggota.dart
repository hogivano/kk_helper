...
class _kurangAnggotaState extends State<KurangAnggota>{
  ...
  List<AnggotaKurang> listKurangAnggota = [];
  ...
  bool _cekInputForm1(){
    if (kepala == "" || noKK == "" || alamat == "" || rt == "" || rw == "" ||
      kelurahan == "" || kecamatan == "" || _jumlahKurang <= 0 || _jumlahAnggota <= 0){
      ...
      return false;
    } else if (_jumlahAnggota <= _jumlahKurang) {
  ...
  bool _cekInputForm2(){
    bool cek = true;
    for(int i = 0; i < _jumlahAnggota ; i++){
      if (listDataKeluarga[i].nama == "" || listDataKeluarga[i].noKtpPen == "" || listDataKeluarga[i].statusHub == ""){
        Fluttertoast.showToast(msg: "Semua data anggota keluarga harus diisi", toastLength: Toast.LENGTH_SHORT);
        cek = false;
      } else if (listDataKeluarga[i].noKtpPen.length != 16){
  ...
  bool _cekInputForm4(){
    ...
    if (_imageKK == null){
      ...
      cek = false;
    } else if ((_imageKeteranganPindah.length + _imageKematian.length) < _jumlahKurang){
              ...
              children: <Widget>[
                new Container(
                  ...
                  child: new DropdownButtonHideUnderline(
                    child: new DropdownButton(
                      hint: new Text(
                        _jumlahKurang == 0 ? "pengurangan" : _jumlahKurang.toString(),
                        style: TextStyle(
                            fontSize: fontForm - 3
                        ),
                        textAlign: TextAlign.center,
  ...
  List<Widget> _showForm2(double fontForm){
    ...
    if (listDataKeluarga.length == 0){
      for(int i = 0; i < _jumlahAnggota; i++){
        _namaKeluargaController.add(new TextEditingController());
        _nikKeluargaController.add(new TextEditingController());

        listDataKeluarga.add(new DataKeluarga());
      }
    } else {
      if (listDataKeluarga.length < _jumlahAnggota) {
        int jumlah = _jumlahAnggota - listDataKeluarga.length;
    ...
    for(int i = 1; i <= _jumlahAnggota; i++){
      list.add(
          new ExpansionTile(
            ...
            children: <Widget>[
              new Container(
                ...
                alignment: Alignment.center,
                child: new TextField(
                  controller: _namaKeluargaController[i-1],
                  textAlign: TextAlign.start,
                  style: TextStyle(
                    fontSize: fontForm,
                    color: Colors.black54,
                ...
                alignment: Alignment.center,
                child: new TextField(
                  controller: _nikKeluargaController[i-1],
                  textAlign: TextAlign.start,
                  style: TextStyle(
                    fontSize: fontForm,
                    color: Colors.black54,
                ...
                child: new DropdownButtonHideUnderline(
                  child: new DropdownButton(
                    hint: new Text(
                      listDataKeluarga[i-1].statusHub == "" ? "Status Hubungan" : listDataKeluarga[i-1].statusHub,
                      style: TextStyle(
                          fontSize: fontForm - 3
                      ),
                      textAlign: TextAlign.center,
  ...
  List<Widget> _showForm3(double fontForm){
    ...
    if (listKurangAnggota.length == 0){
      for(int i = 0; i < _jumlahKurang; i++){
        ...
        listKurangAnggota.add(new AnggotaKurang());
      }
    } else {
      if (listKurangAnggota.length < _jumlahKurang) {
        int jumlah = _jumlahKurang - listKurangAnggota.length;
        for (int i = 0; i < jumlah; i++) {
          _namaAnggotaController.add(new TextEditingController());
          _nikAnggotaController.add(new TextEditingController());
          _alasanKurangController.add(new TextEditingController());
    ...
    for (int i = 1; i <= _jumlahKurang; i++){
      list.add(
        new ExpansionTile(
          initiallyExpanded: false,
          title: new Text(
              listKurangAnggota[i-1].nama == "" ? i.toString() + ".Pengurangan Anggota" : i.toString() + "." + listKurangAnggota[i-1].nama
          ),
          children: <Widget>[
            new Container(
              alignment: Alignment.centerLeft,
