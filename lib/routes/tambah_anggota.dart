...
class _TambahAnggotaState extends State<TambahAnggota> {
  ...
  List<AnggotaTambah2> listTambahAnggota = [];

  File _imageKK;
  File _imagePengantarRtRw;
  List<File> _imageKKLama = [];
  List<File> _imageAktaKelahiran = [];
  List<File> _imageKawin = [];

  String _urlImageKK = "";
  String _urlImagePengantarRtRw = "";
  List<String> _urlImageKKLama = [];
  List<String> _urlImageAktaKelahiran = [];
  List<String> _urlImageKawin = [];

  String kondisiKirim = "";

  _storeData() async {
    DatabaseReference database =
        _database.reference().child("tambah_anggota").push();
    var kurang = [];
    ...
    _storeUpload().whenComplete(() {
      ...
      for (int i = 0; i < _jumlahTambah; i++) {
        kurang.add({
          'nama': listTambahAnggota[i].nama,
          'nik': listTambahAnggota[i].nik,
          'alasanKurang': listTambahAnggota[i].alasanKurang
        });
      }
      database.set({
        ...
        'kecamatan': kecamatan,
        'kelurahan': kelurahan,
        'date': new DateTime.now().year.toString() +
            "-" +
        ...
        'tambahAnggota': kurang,
        'image': {
          'kk': _urlImageKK,
          'pengantarRtRw': _urlImagePengantarRtRw,
          'KKLama': _urlImageKKLama,
          'aktaKelahiran': _urlImageAktaKelahiran,
          'kawin' : _urlImageKawin,
        }
      }).whenComplete(() {
        setState(() {
          kirimData = false;
        });
        Fluttertoast.showToast(msg: "Semua data berhasil dikirim")
            .whenComplete(() {
          Fluttertoast.showToast(
              msg: "Silahkan tunggu acc dari kelurahan dan kecamatan",
  ...
  _storeUpload() async {
    ...
    final StorageReference firebaseKK =
        FirebaseStorage.instance.ref().child("tambahAnggota_kk_" + rand);
    ...
    final StorageReference firebaseKetRTRW = FirebaseStorage.instance
        .ref()
        .child("tambahAnggota_pengantarRTRW_" + rand);
    ...
    for (int i = 0; i < _imageAktaKelahiran.length; i++) {
      StorageReference firebaseRef = FirebaseStorage.instance
          .ref()
          .child("tambahAnggota_aktaKelahiran_" + i.toString() + "_" + rand);
      await firebaseRef.putFile(_imageAktaKelahiran[i]).onComplete.then(
          (kawin) {
        kawin.ref.getDownloadURL().then((url) {
          this._urlImageAktaKelahiran.add(url.toString());
        });
      }, onError: (err) {
        setState(() {
          kirimData = false;
    ...
    for (int i = 0; i < _imageKKLama.length; i++) {
      StorageReference firebaseRef = FirebaseStorage.instance
          .ref()
          .child("tambahAnggota_KKLama_" + i.toString() + "_" + rand);
      await firebaseRef.putFile(_imageKKLama[i]).onComplete.then((kawin) {
        kawin.ref.getDownloadURL().then((url) {
          this._urlImageKKLama.add(url.toString());
        });
      }, onError: (err) {
        setState(() {
          kirimData = false;
    ...
    for (int i = 0; i < _imageKawin.length; i++) {
      StorageReference firebaseRef = FirebaseStorage.instance
          .ref()
          .child("tambahAnggota_kawin_" + i.toString() + "_" + rand);
      await firebaseRef.putFile(_imageKawin[i]).onComplete.then((kawin) {
        kawin.ref.getDownloadURL().then((url) {
          this._urlImageKawin.add(url.toString());
        });
      }, onError: (err) {
        setState(() {
          kirimData = false;
    ...
    for (int i = 0; i < 1; i++) {
      StorageReference firebaseRef =
          FirebaseStorage.instance.ref().child("tambahAnggota_dummy");
  ...
  _showConfirmKirim() {
    return showDialog(
          ...
          builder: (context) => new AlertDialog(
                ...
                content: new Text(
                    'Pastikan data kamu telah benar dan tidak ada yang salah'),
                actions: <Widget>[
                  new FlatButton(
                    onPressed: () => Navigator.of(context).pop(false),
                    child: new Text(
  ...
  getImageKKLama() async {
    ...
    if (image != null) {
      if (image.lengthSync() > 10000000) {
        ...
        setState(() {
          _imageKKLama.add(image);
        });
      }
    }
  }

  getImageAktaKelahiran() async {
    var image = await ImagePicker.pickImage(source: ImageSource.gallery);
    if (image != null) {
      if (image.lengthSync() > 10000000) {
        Fluttertoast.showToast(
  ...
  Future<bool> _showHelpAktaKelahiran() {
    return showDialog(
          context: context,
          builder: (context) => new AlertDialog(
  ...
  bool _cekInputForm1() {
    if (kepala == "" ||
        ...
        _jumlahAnggota <= 0) {
      Fluttertoast.showToast(
          msg: "semua data harus diisi", toastLength: Toast.LENGTH_SHORT);
      return false;
  ...
  bool _cekInputForm2() {
    bool cek = true;
    for (int i = 0; i < _jumlahTambah; i++) {
      if (listTambahAnggota[i].nama == "" ||
          listTambahAnggota[i].alasanKurang == "") {
        Fluttertoast.showToast(
            msg: "Semua data anggota harus diisi",
            toastLength: Toast.LENGTH_SHORT);
        cek = false;
      } else if (listTambahAnggota[i].alasanKurang == "perpindahan") {
        if (listTambahAnggota[i].nik == "") {
          Fluttertoast.showToast(
              msg: "No nik harus diisi", toastLength: Toast.LENGTH_SHORT);
          cek = false;
        } else if (listTambahAnggota[i].nik.length != 16) {
          Fluttertoast.showToast(
              msg: "No nik harus 16 digit", toastLength: Toast.LENGTH_SHORT);
          cek = false;
        }
      } else if (listTambahAnggota[i].alasanKurang == "kelahiran") {
        listTambahAnggota[i].setNik = "";
        _nikAnggotaController[i].text = "";
      }
  ...
  bool _cekInputForm3() {
    ...
    if (_imageKK == null) {
      ...
      cek = false;
        ...
        _jumlahTambah) {
      Fluttertoast.showToast(
              msg: "upload bukti akta kelahiran/kk lama masih kurang")
          .whenComplete(() {
        Fluttertoast.showToast(
            msg:
                "setiap anggota yang dikurangi harus ada bukti akta kelahiran/akta lama");
      });
      cek = false;
    }
      ...
      new Container(
        ...
        child: new DropdownButtonHideUnderline(
          child: new DropdownButton(
            ...
            items: posKecamatan == -1
                ? null
                : listKelurahan[posKecamatan].map((String value) {
                    return new DropdownMenuItem<String>(
                      value: value,
                      child: new Text(value),
                    );
  ...
  List<Widget> _showForm2(double fontForm) {
    ...
    if (listTambahAnggota.length == 0) {
      for (int i = 0; i < _jumlahTambah; i++) {
        ...
        listTambahAnggota.add(new AnggotaTambah2());
      }
    } else {
      if (listTambahAnggota.length < _jumlahTambah) {
        int jumlah = _jumlahTambah - listTambahAnggota.length;
    ...
    for (int i = 1; i <= _jumlahTambah; i++) {
      list.add(new ExpansionTile(
        ...
        title: new Text(listTambahAnggota[i - 1].nama == ""
            ? i.toString() + ".Data Anggota"
            : i.toString() + "." + listTambahAnggota[i - 1].nama),
        children: <Widget>[
          new Container(
            alignment: Alignment.centerLeft,
          ...
          new Container(
            ...
            child: new TextField(
              ...
              onChanged: (val) {
                setState(() {
                  listTambahAnggota[i - 1].setNama = val;
                });
              },
            ),
            padding: const EdgeInsets.fromLTRB(10.0, 0.0, 10.0, 0.0),
          ...
          new Container(
            ...
            child: new TextField(
              ...
              decoration: new InputDecoration(
                ...
                hintText: "isi nik jika alasan perpindahan",
                border: InputBorder.none,
              ),
              keyboardType: TextInputType.number,
              onChanged: (val) {
                setState(() {
                  listTambahAnggota[i - 1].setNik = val;
                });
              },
            ),
            padding: const EdgeInsets.fromLTRB(10.0, 0.0, 10.0, 0.0),
          ...
          new Container(
            ...
            child: new DropdownButtonHideUnderline(
              child: new DropdownButton(
                hint: new Text(
                  listTambahAnggota[i - 1].alasanKurang == ""
                      ? "alasan"
                      : listTambahAnggota[i - 1].alasanKurang,
                  style: TextStyle(fontSize: fontForm - 3),
                  textAlign: TextAlign.center,
                ),
                items: ["perpindahan", "kelahiran"].map((String value) {
                  return new DropdownMenuItem<String>(
                    value: value,
                    child: new Text(value),
                  );
                ...
                onChanged: (value) {
                  setState(() {
                    listTambahAnggota[i - 1].setAlasanKurang = value;
                  });
                },
              ),
            ),
    ...
    return <Widget>[
              ...
              new Row(
                children: <Widget>[
                  ...
                  IconButton(
                      ...
                      onPressed: _showHelpKKLama),
                ],
              ),
              new Container(
                child: _imageKKLama.length == 0
                    ? new Text("Gambar belum dipilih")
                    : _imageKKLama.length == 1
                        ? new Stack(
                            children: <Widget>[
                              new Container(
                                width: MediaQuery.of(context).size.width,
                              ...
                              new Container(
                                child: new IconButton(
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      _imageKKLama.removeAt(_imageKKLama
                                          .indexOf(_imageKKLama.first));
                                    });
                                  },
                                  splashColor: Colors.transparent,
                                ),
                                    ...
                                    children: <Widget>[
                                      ...
                                      new Container(
                                        child: new IconButton(
                                          ...
                                          onPressed: () {
                                            setState(() {
                                              _imageKKLama.removeAt(
                                                  _imageKKLama.indexOf(file));
                                            });
                                          },
                                          splashColor: Colors.transparent,
                                        ),
          ...
          child: new Column(
            children: <Widget>[
              new Row(
                children: <Widget>[
                  new Text(
                    "Akta Kelahiran (Jika Baru Lahir)",
                    textAlign: TextAlign.left,
                    style: new TextStyle(
                        color: Colors.black,
                        fontWeight: FontWeight.bold,
              ...
              new Container(
                child: _imageAktaKelahiran.length == 0
                    ? new Text("Gambar belum dipilih")
                    : _imageAktaKelahiran.length == 1
                        ? new Stack(
                            children: <Widget>[
                              new Container(
                                width: MediaQuery.of(context).size.width,
                              ...
                              new Container(
                                child: new IconButton(
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      _imageAktaKelahiran.removeAt(
                                          _imageAktaKelahiran.indexOf(
                                              _imageAktaKelahiran.first));
                                    });
                                  },
                                  splashColor: Colors.transparent,
                        ...
                        : new CarouselSlider(
                            items: _imageAktaKelahiran.map((file) {
                              return new Builder(
                                builder: (BuildContext context) {
                                  return new Stack(
                                    children: <Widget>[
              ...
              new RaisedButton(
                onPressed: getImageAktaKelahiran,
                color: Color(0xff30a8ff),
                splashColor: Colors.white10,
                elevation: 4,
                shape: new RoundedRectangleBorder(
      ...
      new Container(
          ...
          child: new Column(children: <Widget>[
            ...
            new Container(
              child: _imageKawin.length == 0
                  ? new Text("Gambar belum dipilih")
                  : _imageKawin.length == 1
                  ? new Stack(
                children: <Widget>[
                  new Container(
                    width: MediaQuery.of(context).size.width,
                  ...
                  new Container(
                    child: new IconButton(
                      ...
                      onPressed: () {
                        setState(() {
                          _imageKawin.removeAt(
                              _imageKawin.indexOf(_imageKawin.first));
                        });
                      },
                      splashColor: Colors.transparent,
                    ),
                          ...
                          new Container(
                            child: new IconButton(
                              ...
                              onPressed: () {
                                setState(() {
                                  _imageKawin
                                      .removeAt(_imageKawin.indexOf(file));
                                });
                              },
                              splashColor: Colors.transparent,
                            ),
                    ...
                    child: new Column(
                      ...
                      children: <Widget>[
                        new Container(
                          ...
                          padding:
                              const EdgeInsets.fromLTRB(15.0, 25.0, 15.0, 15.0),
                        ),
                        new Container(
                        ...
                        new Container(
                          ...
                          child: new Stack(children: _btnNextPrev(posForm)),
                        )
                      ],
                    ),
                  ),
